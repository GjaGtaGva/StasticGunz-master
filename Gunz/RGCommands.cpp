...
void LoadRGCommands(ZChatCmdManager& CmdManager)
{
	...
	CmdManager.AddCommand(0, "s", [](const char *line, int argc, char ** const argv) {
		...
		ZChatOutputF("Positon saved at %f, %f, %f", playerPoss.x, playerPoss.y, playerPoss.z);
	},
		CCF_ALL, 0, 0, true, "/s", "");
	...
	CmdManager.AddCommand(0, "tpby", [](const char *line, int argc, char ** const argv) {
		rvector pos = ZGetGame()->m_pMyCharacter->GetPosition();
		ZGetGame()->m_pMyCharacter->SetPosition(rvector(pos.x + atof(argv[1]), pos.y + atof(argv[2]), pos.z + atof(argv[3])));
	},
		CCF_ALL, 3, 3, true, "/tpby <x> <y> <z>", "");
